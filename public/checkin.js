...
export default async function handler(req, res) {
  ...
  try {
    // 1. Validar API Key (permitir admin y public)
    const apiKey = req.headers['x-api-key'];
    const validKeys = [
      process.env.API_KEY_CHECKIN,           // Key administrativa
      process.env.API_KEY_PUBLIC,           // Key pública para frontend
      'cc_checkin_2025_public_frontend'     // Fallback hardcoded
    ].filter(Boolean);
    
    if (!apiKey || !validKeys.includes(apiKey)) {
      console.error('❌ API Key inválida:', apiKey?.substring(0, 10) + '...');
